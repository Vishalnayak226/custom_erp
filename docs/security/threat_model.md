...
| 9 | **Release artifact and migrations** (`erp-server` binary, `db/migration.sql`, 150 incremental migrations) | Build host → `/opt/erp` | Ship a back door to every deployment at once. | Migration checksums and a pending-migration warning at boot. Hermetic CI build, dependency ledger and an unsigned SBOM/checksum/provenance manifest exist since 2026-09-06 (49.9.6/49.9.7, `cmd/releasemanifest`); no signing key, and `deploy/deploy.ps1` does not yet consume the manifest. |
...
| P7 | **Developer or CI account compromise** | Push access, or control of a build | Ship a back door to every deployment | Partial since 2026-09-06: pinned dependencies/toolchain/CI actions, minimal CI token permissions, a hermetic release build and an unsigned SBOM/checksum/provenance manifest (49.9.4/49.9.6/49.9.7/49.9.8). Still weak: no signing key, no branch protection enforcing `.github/CODEOWNERS` yet (R-13), and `deploy.ps1` does not verify the CI artifact. |
...
| M-11 | P7 | Ship a back door | Compromise a dependency, a contributor account or one CI job | Pinned dependencies, signed artifacts, deployment verifies provenance | Release manifest and hash comparison | Partially closed 2026-09-06 (Stage 49.9): dependency ledger + pinned Go modules/CI actions/tool versions, minimal CI token permissions, hermetic release build, and an SBOM/checksum/provenance manifest (`cmd/releasemanifest`) all exist and are tested. Still open: no signing key (nothing to provision one), `deploy/deploy.ps1` does not yet fetch/verify the CI-built artifact, and branch protection/CODEOWNERS enforcement is a pending GitHub setting (R-13). See `docs/security/secure-development-lifecycle.md`. |
