...
> The [rebuilt requirements draft](business-requirements.md) is now available for review. This legacy document remains for content mapping and historical provenance.
...
6. **Error-proof by construction, not by discipline** — the planning references are explicit that the system itself, not user carefulness, must prevent duplicate invoices, over-receipt, negative stock, unauthorized edits, and incomplete transactions. This shows up in the PRD as an explicit error-proofing requirement per module, not an afterthought. As of Stage 23 (2026-07-20) this is also standardized end to end — a single 300+ code message catalog (one standard wording, severity, and HTTP status per scenario, across every module) drives every API error response; see [`../specs/message_catalog.md`](../specs/message_catalog.md).
