...
**Standardized error/message catalog** (Stage 23, 2026-07-20): every API error response is now backed by a generated 300+ code catalog (`internal/server/error_catalog_generated.go`, source-of-truth `ERP_Standard_Message_Control_Matrix_Final.xlsx`, see `docs/specs/message_catalog.md`) with a standard user message, HTTP status, severity, and log/audit requirement per code. This closed a real, live bug: `apiMiddleware` sets `Content-Type: application/json` on every response, but many handlers previously used plain-text `http.Error(...)`, which every frontend `await res.json()` call silently failed to parse — the user saw a generic fallback message instead of the real one.
