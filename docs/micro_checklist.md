...
## Stage 24 — Security & Loopholes Hardening ✅ (32 of 34 items done, 2026-07-22; 2 more found 2026-07-24, see addendum)
...
- [ ] **26.12.1** Order Engine: multi-level Order/Order-Line/Fulfillment/Shipment status (today `POSCart` carries one status field only), plus an order-level Hold engine (reason, owner, release) and a stage-gated cancellation matrix (allowed pre-reservation/pre-dispatch, restricted after pick/pack, blocked after dispatch — route to return/RTO instead). **[needs design decision: new `SalesOrder`/`SalesOrderLine` doctype pair vs. extending `POSCart` with a line-status-history array]** — reference doc §2/§3/§8. Design note (validated by a retired prototype, see `docs/specs/oms_master_blueprint_reference.md` §12): validate in a fixed order (SKU-mapping → address → payment) with a distinct exception code per failure so a hold queue can route by reason, and have hold-release re-run that same validation chain rather than a bespoke resume path. **Effort: L (~1.5-2 sessions)** — biggest single item; everything else below builds on this.
- [ ] **26.12.2** Allocation/Sourcing: extend `FindBestFulfillmentNode` (`engines/sourcing.go:11`, currently one strategy — highest total ATS, hardcoded `"HO"` fallback) with configurable strategies (nearest-pincode, workload, oldest-stock, split-shipment), an `allocation_rule` priority-ordered config doctype, and an allocation-exception queue for when no plan is possible — reference doc §5. Design note (§12): a retired prototype used pincode-difference as a cheap nearest-node proxy (a real geo/zone lookup would be a genuine improvement, not just a port); reallocation should reserve-at-new-location-then-verify and roll back to the old reservation on failure, not verify-then-reserve. **Effort: M (~0.75-1 session)**.
- [ ] **26.12.3** Fulfillment: split `FulfillmentTask` (`engines/fulfillment.go:81`, currently one stage: Pending→Rejected/Dispatched) into distinct Pick and Pack stages with scan-first validation (wrong-barcode/duplicate-scan blocked) and short-pick reason capture. Cross-check `engines/wms.go`'s existing bin-level pick-list structure first — reuse, don't duplicate. Reference doc §6. Design note (§12): match a scan to the first task line still short of expected qty and name the actual product in the rejection message when it doesn't match; block pack completion on any exact-qty mismatch (not just a shortfall). **Effort: M (~1 session)** — cross-checking `wms.go` first is what keeps this from ballooning into a rebuild.
- [ ] **26.12.4** Courier/Shipment/Manifest: extend `CreateLogisticsBooking` (`engines/marketplace.go:10`, currently a bare manual carrier/tracking/charge record) into a real Shipment engine — serviceability check, AWB assignment, label, manifest grouping, RTO detection — via the same outbox/provider-connector pattern already proven for Shopify/BigCommerce/Magento/Unicommerce (`engines/connector_*.go`, `engines/unicommerce.go`), not a new integration mechanism. Reference doc §7 flags this as the single largest orchestration gap versus the blueprint. Design note (§12): a parent order should only flip to Shipped once *every* fulfillment order under it reaches Dispatched/Closed (otherwise Partially Fulfilled) — the concrete split-shipment-aware closure rule the blueprint only states abstractly. **Effort: L (~1.25-1.5 sessions)** — the single largest orchestration gap; build the internal AWB/manifest engine first and treat a real courier API as a separate, later, credentials-gated item (same pattern as 26.2.1-26.2.5).
...
- [ ] **26.12.6** Inventory buckets: add Blocked/QC-Hold/Damaged/Channel-Buffer columns to `inventory_availability` (additive) to complete the ATP formula — today's `GetAvailableToSell` (`engines/inventory.go:255`) computes ATS from only 3 of the blueprint's 7 terms (Available − Reserved − Safety Stock). Reservation concurrency itself is already correct (`FOR UPDATE` locking, `engines/inventory.go:215-228`) and does not need to change. Reference doc §4. **Effort: S (~0.5 session)** — additive columns + formula update; do early since 26.12.5's QC disposition and 26.12.4's RTO handling both want these buckets to already exist.
...
- [ ] **26.12.9** Configuration masters: `allocation_rule`, `reason_code`, `status_transition_rule` as ordinary doctypes on the existing doctype-meta engine — no new mechanism required. Reference doc §11. **Effort: S (~0.25-0.5 session)** — do early alongside 26.12.6; 26.12.2's strategies need `allocation_rule` and 26.12.1's cancellation matrix needs `reason_code`.
