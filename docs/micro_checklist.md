...
- [ ] **18.2 Data-Entry Integrity (Relational Constraints)** — The known gap regarding free-text fields for master data instead of proper relation lookups is a critical flaw that will cause severe data corruption if real users start using it.
...
## Stage 20 — ERP Maturity Master Plan Execution ⏳ (mostly — 8 items open: Track A + 20.30/20.31)
...
- [ ] **20.6** Zero Real-World Production Validation — The system is feature-rich but has never been tested in a live production environment with real user load and messy data. This is the biggest immediate risk.
...
- [ ] **21.14** The "Vanilla JS" Ceiling & Ultra-Fast UI — Adopt a two-pronged frontend strategy to raise the complexity ceiling and achieve a "PDF-like" instant feel, while strictly honoring the no-build-step first principle. 1) **JS Structure:** Migrate to Native Web Components to encapsulate logic and prevent spaghetti code without needing a framework. 2) **UI Speed:** Implement Stale-While-Revalidate (SWR) caching via `sessionStorage` and optimistic UI updates to eliminate loading spinners and layout shifts.
...
## Stage 24 — Security & Loopholes Hardening ⏳ (32 of 34 items done, 2026-07-22; 6 more found 2026-07-24/25, see addendum)
...
**Two genuine gaps found**, not covered by anything above:
- [ ] **24.33** `_ = json.Unmarshal(...)` (ignored error) has crept back into production code since 24.18 — 5 current occurrences, two with real fail-open risk: `engines/transactional_validation.go:85` (malformed `PurchaseOrder.items` silently computes zero ordered-qty, which could let an over-receipt check pass incorrectly) and `internal/server/handlers_core_doc_engine.go:449` (malformed prior document data silently feeds `ValidateTransactionalRules` an empty prior-state, which could let an invalid status transition through). `engines/transactional_validation.go:212` and `engines/pim_content_versions.go:67` are lower-risk (skip one business-rule check silently / display-only history read). `internal/server/handlers_pim_pos_finance.go:130` is not a real risk — it unmarshals JSON the same function just marshaled itself 3 lines above, guarded by `marshalErr == nil`. Fix the two real ones; leave the harmless one alone.
- [ ] **24.34** No connection pool monitoring/metrics exists anywhere (confirmed via grep — no `db.Stats()` call, no `/metrics`/pool-stats endpoint). 24.13 tuned the pool's bounds but exposes no visibility into it. Cheapest fix: surface `db.DB.Stats()` (open/idle/in-use connections, wait count) on the existing health-check endpoint (24.14) or a new admin-only route — no new dependency needed, `database/sql` already exposes this.

### Addendum (2026-07-25): Code Audits & Architecture Loopholes
- [ ] **24.35** Inbound webhook fail-open risk: `verifyShopifyWebhookSignature` bypasses signature verification completely if `SHOPIFY_WEBHOOK_SECRET` is unset, exposing the endpoint to unauthenticated spoofing.
- [ ] **24.36** Offline POS Idempotency risk: LocalStorage queue allows negative-stock pushing and untraceable cash theft if a cashier clears local browser storage before reconnecting.
- [ ] **24.37** Stage 18 Gap (Free-text fields): Despite Typeahead being added, actual master data referential constraints are missing. Pickers are also unusable by Store Manager/Cashier, leaving data integrity at risk of garbage free-text entry.
- [ ] **24.38** MFA Physical bypass: `cmd/reset_mfa` allows any host-level shell access to bypass application-level MFA checks and clear Admin/HR protections.
...
**Build started 2026-07-24**: the two foundational items (26.12.9, 26.12.6), 26.12.1 (Order Engine), 26.12.2 (Allocation/Sourcing), 26.12.3 (Fulfillment Pick/Pack), and 26.12.4 (Courier/Shipment/Manifest, built out of numbering order ahead of 26.12.5 in a concurrent-session pass since it doesn't depend on 26.12.5 and touches a disjoint file, `engines/marketplace.go`) are done — see their own entries below for detail. 26.12.5 (Returns/RTO/QC/Refund) remains open next; 26.12.7/26.12.8 (dashboard/reports) are next after that, now that 26.12.1-26.12.6 all have real data for them to read.
...
- [ ] **26.12.5** Returns/RTO/QC/Refund: extend `ProcessReturnAnywhere` (`engines/fulfillment.go:291`, currently an unconditional same-step receive-and-restock-and-refund) with a request/approval step before receipt, QC disposition buckets (Sellable/Damaged/Repairable/Missing/Wrong-Item/Rejected) driving which inventory bucket a returned item lands in, an RTO path for courier-returned undelivered shipments, and a refund-request record distinct from the immediate GL post. Reference doc §9. Design note (§12): compute refund amount from the *original order line's* price, not the return-time price; decide explicitly whether one QC-failed line holds the whole refund or only that line (a retired prototype did the former, which forks against the blueprint's own line-level-refund principle — don't inherit silently). **Effort: M/L (~1 session)**.
...
- [ ] **26.12.7** Exception/reconciliation dashboard: extend Stage 26.10.5's planned exception-queue widget to cover OMS events over the existing `integration_event_outbox`/`integration_event_log` (`engines/outbox.go`, `engines/unicommerce.go:295`) — no new retry/DLQ mechanism needed, just OMS-scoped visibility — plus a new reconciliation-variance report (OMS vs. channel/courier). Reference doc §10. **Effort: S/M (~0.5-0.75 session)**.
- [ ] **26.12.8** OMS reports: Order Aging, SLA Breach, Allocation Pending, Stock Mismatch, Return Aging, Reserved Stock, Courier Performance — new `ReportDefinition` entries, same pattern as the existing Stock/Sales/Vendor-Ledger/Payables reports (Stage 20 Track B.4). Reference doc §11. **Effort: M (~0.75-1 session)** — 7 reports, but the framework is proven; build last since each one reads off data 26.12.1-26.12.6 produce.
...
- [ ] **26.12.10** [P2 — tier/scope decision, don't build speculatively] Per-order/customer notification templates (email/SMS/WhatsApp) distinct from `alerting.go`'s incident-level Slack/Teams alerts — needs a product decision on customer-facing communication scope before building. Reference doc §11.
...
- [ ] **27.8** [fast-follow, deliberately not built this pass] An admin UI screen for picking packages per tenant (a picker over `engines.ProductPackages` that calls the existing `POST /api/v1/admin/tenant/module-entitlement`/the new `packages` field on `handleProvisionTenant`) — item **26.1.4** above already tracks exactly this gap; this Stage (27.1-27.7) made the mechanism underneath it correct and product-shaped, so this is now a pure UI-only item with no backend work left, not a design unknown.
