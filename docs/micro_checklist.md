...
- [ ] **26.12.7** Exception/reconciliation dashboard: extend Stage 26.10.5's planned exception-queue widget to cover OMS events over the existing `integration_event_outbox`/`integration_event_log` (`engines/outbox.go`, `engines/unicommerce.go:295`) — no new retry/DLQ mechanism needed, just OMS-scoped visibility — plus a new reconciliation-variance report (OMS vs. channel/courier). Reference doc §10.
- [ ] **26.12.8** OMS reports: Order Aging, SLA Breach, Allocation Pending, Stock Mismatch, Return Aging, Reserved Stock, Courier Performance — new `ReportDefinition` entries, same pattern as the existing Stock/Sales/Vendor-Ledger/Payables reports (Stage 20 Track B.4). Reference doc §11.
- [ ] **26.12.9** Configuration masters: `allocation_rule`, `reason_code`, `status_transition_rule` as ordinary doctypes on the existing doctype-meta engine — no new mechanism required. Reference doc §11.
