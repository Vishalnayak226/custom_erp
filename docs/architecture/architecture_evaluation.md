...
### 6.3 Standardized Error Response Envelope (Stage 23, 2026-07-20)
Every API error response now goes through `internal/server/apierror.go`'s `writeAPIError`/`writeAPIErrorGeneric`, which write the same JSON shape everywhere: `{"error": "<standard message>", "code": "<catalog code, e.g. MASTER-0040>", "correlation_id": "<uuid>", "retryable": <bool>}`. This closed a real gap: `apiMiddleware` sets `Content-Type: application/json` on every response, but the previous ad-hoc `http.Error(w, "text", status)` calls wrote plain text, which every frontend `await res.json()` caller silently failed to parse. The codes themselves come from a generated catalog (`internal/server/error_catalog_generated.go`) — see **[docs/specs/message_catalog.md](../specs/message_catalog.md)** for the full design and source-of-truth xlsx. `LogSystemError`/`LogAuditEvent` (§6.1) still do the actual logging; the catalog just decides, per code, whether they fire for a given response (`LogRequired`/`AuditRequired` flags) and tags the logged message with the code.
