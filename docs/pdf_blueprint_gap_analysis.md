...
| **Vendor / Customer as real masters** | MB §4.5 (dedicated Vendor/Customer/Supplier masters with GSTIN/bank/loyalty fields) | **[unverified, likely partial]** `db/migration.sql` seeds no `Vendor` or `Customer` doctype — `vendor` and `customer` only exist as free-text `Data` fields on `PurchaseOrder`/`SalesInvoice` (`db/migration.sql:333,350`). The sidebar has a "Vendors" menu item, so a `Vendor` doctype may have been created later via the runtime DocType Builder (not visible in migration seed data) — this needs a live-DB check (`SELECT * FROM tenant_default.doctype_meta WHERE name IN ('Vendor','Customer')`) rather than a source-code check. Flagging rather than asserting. |
...
11. Dedicated Vendor/Customer masters (replacing the free-text fields) with GSTIN/bank/loyalty fields per MB §4.5 — pending the live-DB check in §4 above to see what already exists.
