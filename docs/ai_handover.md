...
*   **Dev Server State**: `erp-server.exe` and the portable Postgres instance (port 5435) were both left running at end of this session (Stage 18 work was live-verified against them directly, no throwaway instance needed since it's a frontend/SQL-only change with no Go rebuild). Confirm with `netstat -ano | findstr "5435 8080"` before assuming either is down.
*   **Verification Status**: Stage 18 (this session) touched only `public/app.js`, `public/styles.css`, and a new SQL migration — no Go files changed, so `go build`/`vet`/`test` were not re-run this session; the Stage 19 result below (last time Go code changed) still stands. `node --check public/app.js` clean. Live-verified via Playwright against the real running dev server + `custom_erp` DB (see `docs/project_ledger.md` §16) rather than a throwaway instance, since there was nothing to rebuild. **Uncommitted at time of writing**, on top of commit `123f093` (which already closed Stage 17.10-17.11 + Stage 19): `docs/ai_handover.md`/`micro_checklist.md`/`project_ledger.md`, `public/app.js`, `public/styles.css`, and new `db/migrations_stage18_core_module_fix.sql` (already applied live to `custom_erp` — see Stage 18 note above; re-applying is idempotent, uses `ON CONFLICT`/`ON CONFLICT ... DO UPDATE`). `public/qrcode.min.js` remains untracked and unrelated (pre-existing, noted in Stage 19).
*   `go build`, `go vet ./...`, and `go test ./... -p 1` were clean as of the Stage 19 work (last Go changes, commit `123f093`). `internal/server/server_test.go`'s integration test completes real MFA enrollment as part of its login step, since the HR/Admin fixture user is MFA-gated. Note (found 2026-07-19): every test file's `db.InitDB` call uses a literal hardcoded connection string to the real `custom_erp` database on port 5435, not an isolated test DB - `DATABASE_URL` has no effect on `go test`. When landing a new migration, apply it to `custom_erp`, not just `custom_erp_test` (that one's only used by the Stage 17.3 backup/restore drill).
*   **Concurrent sessions**: More than one AI session (and the user directly) has worked on this repo in the same window in past sessions (e.g. a stress-test process, a `DecideApproval` TOCTOU fix, and — 2026-07-19 — the user committing Stage 16 close-out directly to `main` mid-session while an AI session was also active). Before staging/committing, always check `git status`/`git diff` and add only the specific files you actually reviewed — never `git add -A` — since another session's in-progress or already-committed work may be sitting in the working tree. **Documentation convention** (see this repo's own `CLAUDE.md`): keep `docs/micro_checklist.md` (what's to be done), `docs/project_ledger.md` (what's been done, chronological), and this file's §6 (handover snapshot) in sync after every unit of work, proactively.
